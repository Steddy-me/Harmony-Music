...
class MyAudioHandler extends BaseAudioHandler with GetxServiceMixin {
  ...
  bool _handlingCompletedLastTrack = false;
  ...
  void _notifyAudioHandlerAboutPlaybackEvents() {
    _player.playbackEventStream.listen((PlaybackEvent event) {
      ...
      final isLastTrack = currentIndex != null &&
      ...
      if (_player.processingState == ProcessingState.completed &&
          ...
          !loopModeEnabled &&
          !queueLoopModeEnabled &&
          !_handlingCompletedLastTrack) {
        endOfQueueReached = true;
        _handlingCompletedLastTrack = true;
        ...
        Future.microtask(() async {
          try {
            ...
            _handlingCompletedLastTrack = false;
          }
        });
      }
  ...
  void _listenToPlaybackForNextSong() {
    ...
    _player.positionStream.listen((value) async {
      if (_player.duration != null && _player.duration?.inSeconds != 0) {
        ...
        if (isLastTrack && !queueLoopModeEnabled && !loopModeEnabled) {
          return;
        }
  ...
  Future<void> _triggerNext() async {
    ...
    if (isLastTrack && !queueLoopModeEnabled) {
      return;
    }
  ...
  AudioSource _createAudioSource(MediaItem mediaItem) {
    final source = _getPlayableSource(mediaItem);

    if (source == null || source.isEmpty) {
      throw Exception('No playable source found for media item ${mediaItem.id}');
    }

    final isFileSource = source.startsWith('file:');
    final isHttpSource =
    ...
    final songs = box.values.toList().map((e) {
      ...
      return MediaItem(
        ...
        extras: {
          ...
          "length": song.extras?['length'],
          "album": song.extras?['album'],
          "artists": song.extras?['artists'],
          "date": song.extras?['date'],
          "trackDetails": song.extras?['trackDetails'],
